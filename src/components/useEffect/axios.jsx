...
const AxiosExample=()=>{
    ...
    <div style={{textAlign:"center"}}>
        <h1>Search Products.....</h1>
        <input type="text" onChange={handle} value={search} style={{ marginBottom: "20px" }}/>
    </div>

    <div style={{width:"100vw",height:"100vh",display:"flex",justifyContent:"center",alignItems:"center",flexWrap:"wrap"}}>
{
        filterData.length > 0 ?
        (
            filterData.map((item)=>{
                return<>
                <BasicExample key={item.id}
                title={item.title}
                description={item.description}
                category={item.category}
                image={item.images}
